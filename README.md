...
- **主工作台**：项目 → 六关（Requirements / Design / Development / Testing / Deployment / Verification）→ 文牒 → 部署 → 时间线的黄金流程；Agent 执行、审批中心与知识库检索在同一工作区。新建任务页为 ZCode 首页式大输入器——胶囊上下文行（工作区/关卡模板）、交付流程随所选模板自适应；新建任务与工作台输入器收编为同一公共组件，`@` 指派 Agent、`/` 选技能全站同交互（"+" 菜单为统一入口，工作台另可 `#` 引用知识源）。
- **关卡 Run 上下文与交付评审**：每关 Run 启动前由服务端固定装配上下文——原始目标 + 本关验收标准 + 必需的上游已批准产物（只认冻结修订），上游缺失或已失效即 fail-closed 拒绝启动，不许 Agent 在不完整上下文里自行猜测；装配清单随 Run 持久化，执行过程「输入」面板经 `agent.gateContext` 逐项展示本次已读取的上游交付物及版本（rev/etag），作为产出核验对照基准；关卡交付评审以验收核对清单逐条对照产出（列明遗漏/冲突/偏离）后再放行。关卡展示元数据（标题/目的/验收要求/执行 Agent）全部来自工作流模板实例，自定义关卡 id 贯穿 UI。
