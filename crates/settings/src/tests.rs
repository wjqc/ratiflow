...
mod tests {
    ...
    fn credential_delete_blocked_by_dependents_force_marks_degraded() {
        ...
        let blocked =
            credentials::remove(&s, &backend, &cred.id, cred.revision, false).unwrap_err();
    ...
    fn model_profile_managed_readonly_and_route_reference() {
        ...
        let updated =
            profiles::model_update(&s, &p.id, &json!({"name":"改名"}), p.revision).unwrap();
        ...
        let err =
            profiles::model_update(&s, "mp_env_model", &json!({"name":"x"}), managed.revision)
                .unwrap_err();
    ...
    fn audit_redaction_and_export() {
        ...
        let seq = audit_ext::append(
            ...
            Some("corr_t1"),
            None,
            Some(&json!({"before": "password = supersecret123"})),
            Some(&json!({"hasSecret": true})),
        )
        .unwrap();
