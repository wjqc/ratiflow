...
pub trait GitLabClient: Send + Sync {
    ...
    fn create_mr(&self, project_id: &str, source: &str, target: &str, title: &str) -> Result<GitLabMR, String>;
    fn get_mr(&self, project_id: &str, iid: &str) -> Result<GitLabMR, String>;
    fn pipelines_for_ref(&self, project_id: &str, gitlab_ref: &str) -> Result<Vec<GitLabPipeline>, String>;
}
...
impl GitLabClient for GitLabHttp {
    ...
    fn get_issue(&self, project_id: &str, iid: &str) -> Result<GitLabIssue, String> {
        ...
        Ok(GitLabIssue {
            iid: raw["iid"].as_i64().map(|v| v.to_string()).unwrap_or_default(),
            title: raw["title"].as_str().unwrap_or_default().into(),
            body: raw["description"].as_str().unwrap_or_default().into(),
            labels: raw["labels"].as_array().map(|a| a.iter().filter_map(|l| l.as_str().map(String::from)).collect()).unwrap_or_default(),
            state: raw["state"].as_str().unwrap_or_default().into(),
        })
    }

    fn create_mr(&self, _project_id: &str, _source: &str, _target: &str, _title: &str) -> Result<GitLabMR, String> {
    ...
    fn pipelines_for_ref(&self, project_id: &str, gitlab_ref: &str) -> Result<Vec<GitLabPipeline>, String> {
        let raw = self.get(&format!("/projects/{project_id}/pipelines?ref={gitlab_ref}"))?;
        let list = raw.as_array().cloned().unwrap_or_default();
        Ok(list.iter().map(|p| GitLabPipeline {
...
impl GitLabClient for FakeGitLab {
    ...
    fn get_mr(&self, _project_id: &str, iid: &str) -> Result<GitLabMR, String> {
        self.mrs.iter().find(|m| m.iid == iid).cloned().ok_or_else(|| "gitlab_not_found".into())
    }

    fn pipelines_for_ref(&self, _project_id: &str, gitlab_ref: &str) -> Result<Vec<GitLabPipeline>, String> {
        Ok(self.pipelines.iter().filter(|p| p.gitlab_ref == gitlab_ref).cloned().collect())
    }
}
